...
from jobradar.core.matcher import location_matches, matches
...
# Shared immutable default: an empty rule imposes no location filter.
_NO_LOCATION_FILTER = MatchRule()
...
class Runner:
    ...
    def __init__(
        ...
        *,
        location_rule: MatchRule = _NO_LOCATION_FILTER,
        max_concurrency: int = 8,
    ) -> None:
        self._sources = sources
        self._rule = rule
        self._location_rule = location_rule
        self._store = store
    ...
    async def run_once(self) -> list[Job]:
        ...
        A job must pass *both* filters: its title matches the keyword rule **and**
        its location matches the location rule (which drops jobs with no location
        when set). Either empty rule imposes no filter on that axis.
        """
        fetched = await self._fetch_all()
        matched = [
            job
            for job in fetched
            if matches(job, self._rule) and location_matches(job, self._location_rule)
        ]
